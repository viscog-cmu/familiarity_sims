...
sys.path.append('.')
from familiarity.familiarization import get_vgg16_pretrained_model_info, get_cornet_z_pretrained_model_info, _make_fbf_net_name, _freeze_layers, _get_optimizer_and_scheduler, train_and_verify
...
parser.add_argument('--id-thresh', type=int, default=20, help=' ')
# parser.add_argument('--n-train', type=int, default=50, help='only for vggface2-test')
parser.add_argument('--n-val', type=int, default=19, help=' ')
# parser.add_argument('--n-test', type=int, default=50, help='only for vggface2-test')
# parser.add_argument('--n-ids', type=int, default=50, help='only for vggface2-test')
parser.add_argument('--max-ids', type=int, default=None, help=' ')
parser.add_argument('--im-dim', type=int, default=224, help=' ') # must be square
parser.add_argument('--batch-size', type=int, default=64, help=' ')
parser.add_argument('--epochs', type=int, default=10, help=' ')
parser.add_argument('--overwrite', action='store_true', help=' ')
# parser.add_argument('--no-freeze-pre-fc', action='store_true', help='fine tune the full network rather than only the FC layers')
# parser.add_argument('--classifier-only', action='store_true', help='fine tune only the output layer')
...
exclude_keys = ['no_gpu', 'dist_layer_inds', 'dist_layers', 'overwrite',
    ...
    'num_workers', 'im_dim', 'no_grow_labels', 'verification_phases',
...
elif 'vggface2-test_subset' in opt['dataset']:
    exclude_keys += ['gender', 'ntrain_thresh', 'id_thresh', 'n_val']
    data_dir = f"{DATA_DIR}/imagesets/{opt['dataset']}/fold-{opt['fold']}"
else:
    raise NotImplementedError('Not yet configured for dataset: {}'.format(opt['dataset']))
if 'bn' not in opt['net']:
    exclude_keys += ['bn_freeze']
for bool_var in ['random_weights', 'no_grow_labels', 'inverted', 'inverted_test', 'data_aug', 'crop5', 'lr_scheduler']:
...
weights_path = None if opt['random_weights'] else f"{DATA_DIR}/from_scratch/models/{opt['net']}.pth"
...
if 'vggface2-test_subset' in opt['dataset']:
...
train_and_verify(model, dataloaders, criterion, base_fn, dist_layers=opt['dist_layers'], imageset_dir=data_dir,
